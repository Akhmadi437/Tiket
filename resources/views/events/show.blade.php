<x-layouts.app>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mt-4 space-y-4">
                            @forelse($event->tikets as $tiket)
                                @php
                                    // ✅ Nama tipe tiket dari relasi TicketType
                                    $namaTipe = $tiket->ticketType?->nama ?? '-';
                                @endphp

                                <div class="card card-side shadow-sm p-4 items-center">
                                    <div class="flex-1">
                                        <h4 class="font-bold">{{ $namaTipe }}</h4>
        ...
        <dialog id="checkout_modal" class="modal">
            <form method="dialog" class="modal-box">
                ...
                <div class="modal-action">
                    ...
                    <button
                        type="button"
                        class="btn btn-primary px-4 !bg-blue-900 text-white"
                        id="confirmCheckout"
                    >
                        Konfirmasi
                    </button>
    ...
    <script>
        (function () {
            ...
            // ✅ tickets object: tipe = nama dari relasi TicketType
            const tickets = {
                @foreach($event->tikets as $tiket)
                    {{ $tiket->id }}: {
                        ...
                        price: {{ $tiket->harga ?? 0 }},
                        stock: {{ $tiket->stok }},
                        tipe: @json($tiket->ticketType?->nama ?? '-'),
                    },
                @endforeach
            };
